		...
		$.each(files, function(index, file) {
			...
			var fileReader = new FileReader();
				...
				fileReader.onload = (function(file) {
					return function(e) {
						var albumID = $("input[name='currentAlbum']").val();
						// Помещаем URI изображения в массив
						dataArray.push({name : file.name, value : this.result, albumID : albumID });
		...
		$.each(dataArray, function(index, file) {	
			...
			$.post('/upload.php', dataArray[index], function(data) {
				...
				if(dataSplit[1] == 'загружен успешно') {
					$('#uploaded-files').append('<li><a href='+dataSplit[0]+'"../images">'+fileName+'</a> загружен успешно</li>');
	...
	$(function(){
		$(".delPhoto").on("click", function(){
			if(this.hasAttribute('photoID')) {
				var photoID = parseInt(this.getAttribute('photoID'));
				var mydataArray = {deletePhoto: photoID };

				$.post("/upload.php", mydataArray, function (data) {
					console.log(data);
